...
### 2026-09-21 — Removed the case-type menu
- Checked first that nothing consumed it: `case_types`, `suggested_skills` and `suggested_roles` were only put in the triage model's input; no code, agent, skill or limit read the chosen `case_type` (the UI just prints it). Removed `case_types` from `config/agents.yaml` and `CaseTypeConfig`/`case_type_map` from `src/config.py`; `Triage` keeps `case_type` as a free short label and drops `case_type_description`, `suggested_skills`, `suggested_roles` and the `novel` validator; the triage prompt no longer mentions a menu. Spec §3 updated (design decision changed).
- Effect on stored runs: the committed showcase runs were produced with the menu. Their events still show the old triage fields (rendered generically), so re-run the ten cases (`inspect eval`) and `inspect showcase-export` to get runs that match the current code. Not done here by decision.
- Verification: `uv run pytest` 68 passed, ruff clean, frontend `tsc -b`, vitest (30) and `npm run e2e` pass.

### 2026-09-21 — Readable inspector and report sections
- Problem: model outputs (investigation summary, hypotheses, key facts, open questions) rendered through a generic nested list, which packed them together; report hypotheses were one run-on paragraph with truncated evidence chips and unstyled ids in the decoy/missing-evidence text.
- `run/Fields.tsx`: `Ref` (id chip), `Prose` (turns graph ids inside sentences into chips; needs a known prefix plus a digit, so "IP-based" stays a word), `Refs` (id chips, extra ones folded); objects render as a two-column label/value grid. `run/ModelOutput.tsx`: dedicated bodies for hypotheses (label, status pill, signed `+` for and `−` against lines, folded after three), facts (statement with its graph ids), the investigation summary (hypotheses, key facts, open questions, contradictions as separate parts), suggested next steps and task plan items. `run/ActorPanel.tsx`: repeated exits shown as "decided by supervisor ×4". `run/Conclusion.tsx`: claim and confidence bar plus a "What would change this decision" callout, hypothesis cards (accepted first, colour rail, serif statement, reasoning, full-width evidence rows that still highlight the graph), decoys/missing evidence/policy/account actions as separated items with id chips. `index.css`: `.ref` and `.stack` rules.
- Verification: `tsc -b`, vitest (31, incl. `Fields.test.tsx`), lint, build, `npm run e2e`; checked in a browser on the stored Family Tablet run. Code-simplifier pass done. Left as is: `Evidence` and `Marked` in `ModelOutput.tsx` are near duplicates; document ids in the policy table still use the plain `id-chip`.

### 2026-09-21 — Inspector showed the adjudicator's hypotheses as bare verdicts
- Bug: the `Hypotheses` renderer in `run/ModelOutput.tsx` read only `label`, `support` and `against` (the working-hypothesis shape). The adjudicator's `HypothesisAssessment` uses `hypothesis`, `status` (accepted/rejected), `why` and `evidence`, so its node in the inspector showed only rows of "accepted"/"rejected" pills. Now the claim is the title, the verdict pill is green or red, `why` follows, and each evidence claim is listed with its graph id chips. The report panel at the bottom was already correct. Test: `run/ModelOutput.test.tsx` (both shapes).
- Verification: `tsc -b`, vitest (33), lint, build, `npm run e2e`; checked in a browser on the stored Family Tablet adjudicator node.
